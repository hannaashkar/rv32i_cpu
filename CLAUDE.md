...
**2026-07-15 — D026–D029 production merge + CI acceptance:**
- The complete verification-hardening, D026 LQ, D027 SQ, D028 IQ, and D029
  bypass/25 MHz stack is now **merged and pushed to production** at RTL
  baseline **`3c3171f`**. The full feature history
  is also backed up remotely as `codex/load-wb-bypass-cut` at **`fe3cfb0`**.
- GitHub Actions production run **29368469898 is green**, confirming the
  repository's directed, ISA, trimmed-random, and lockstep CI gate on the
  merged tree. The deeper local verification, coverage, CoreMark, Quartus
  fit/STA, and assembly evidence above remains the release evidence for D029.
- The freshness-clean PLL-/2 MNIST `.sof`/`.pof` remain **unflashed**. No JTAG
  hardware was available on 2026-07-15, so this merge does not change the
  physical claim: the last hardware-confirmed image is still the 16.67 MHz OoO
  LED walker booting from CFM. The 25 MHz clock and MNIST demo are production
  build artifacts, not yet on-silicon results.

**2026-07-15 — recruiter release package:**
- The canonical recruiter deliverable is now the tracked 8-page
  `output/pdf/rv32i_soc_portfolio.pdf`, generated by
  `scripts/build_portfolio.py` from the machine-readable
  `portfolio/evidence.json` claim ledger. The old ignored `docs/book/` copy is
  retired and must not be shared.
- `docs/CAREER_PACKET.md` contains quantified résumé bullets, LinkedIn copy,
  short project pitches, interview stories, and skeptical follow-up answers.
  README links the package and `make portfolio` reproduces the PDF after
  installing `requirements-portfolio.txt`.
- The package separates simulation, Quartus/STA, hardware-confirmed, and
  pending evidence. In particular, it does not claim Linux support, formal
  verification, a physical 25 MHz run, or a physical MNIST result.

**Next → see `docs/NEXT.md` (the start-here backlog).** In brief:
(0) **Flash the 25 MHz D029 MNIST demo** — `.sof` is built, but no JTAG
hardware was available on 2026-07-15; this remains Hanna's physical-board step
(USB-Blaster → digit demo → video). Do NOT auto-do. (1) ~~PRF→M9K LVT~~
**DONE + merged (D025, `ab79b50`, −8,895 LEs → 70%, IPC-neutral).**
(2)–(5) ~~D026 LQ + D027 SQ + D028 IQ + D029 bypass/PLL work~~ **DONE,
merged, pushed, and CI-green at production RTL baseline `3c3171f`;** fresh 25 MHz
MNIST images remain unflashed. (6) **New
...
loop/call-tree/RAS random coverage, and INV-G2 negative self-test. The
recruiter portfolio package is complete and tracked. **Env note:**
the two old build landmines are
GUARDED IN THE MAKEFILE — `make` just works; if overriding, VERILATOR_ROOT
must be `/ucrt64/share/verilator` (mount form). D024–D029 and verification
hardening are merged/pushed on `main`; the D029 feature stack is backed up at
`origin/codex/load-wb-bypass-cut` (`fe3cfb0`);
