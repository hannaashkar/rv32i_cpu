...
{
    /* rv32i_cpu port: no UART — the Verilator harness snoops the MEM-stage
       store bus and prints any byte stored to the sim-console MMIO address
       (0x40000010, MMIO_SIM_CONSOLE in sw/common/rv32.h). The store lands
       in the MMIO region, where rtl/soc/mmio.v ignores unknown addresses,
       so this is free: no RTL change, and on real hardware it is a no-op.
       No ready/busy handshake needed — the "console" never stalls. */
    *(volatile unsigned int *)0x40000010u = (unsigned char)c;
}
